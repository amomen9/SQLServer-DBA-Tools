...
------------------------------------------------------------
DECLARE 
    @DatabaseName sysname = N'TBS',   -- Target database
    @Execute      bit     = 0,        -- 1 = execute restore chain
    @StopAt       datetime = NULL,    -- Point-in-time inside last DIFF/LOG
    @WithReplace  bit     = 1,        -- Include REPLACE on RESTORE DATABASE
...
SET RestoreCommand =
    CASE rc.BackupType
        WHEN 'FULL' THEN
            ...
            CASE WHEN rc.StepNumber = @LastStep THEN N', RECOVERY;' ELSE N', NORECOVERY;' END
        WHEN 'DIFF' THEN
            N'RESTORE DATABASE [' + @DatabaseName + N'] FROM ' + dc.Disks + N' WITH ' +
            (CASE 
             ...
             END) +
            N'STATS = 5' +
            CASE WHEN rc.StepNumber = @LastStep AND @HasLogs = 0 THEN N', RECOVERY;' ELSE N', NORECOVERY;' END
        WHEN 'LOG' THEN
...
DECLARE 
    @FullInfo nvarchar(200) = (SELECT 'FullFinish=' + CONVERT(varchar(19), backup_finish_date, 120) 
                                      + '; LastLSN=' + CAST(last_lsn AS varchar(40)) FROM #Full),
    @DiffInfo nvarchar(200) = (SELECT 'DiffFinish=' + CONVERT(varchar(19), backup_finish_date, 120) 
                                      + '; LastLSN=' + CAST(last_lsn AS varchar(40)) FROM #Diff),
...
PRINT 'Execution mode: ' + CASE WHEN @Execute = 1 THEN 'EXECUTE' ELSE 'SCRIPT ONLY' END;
PRINT '------------------------------------------------------------------';

------------------------------------------------------------
-- PRINT commands
------------------------------------------------------------
PRINT '--- Generated RESTORE Commands (in order) -------------------------';
...
SELECT 
    ...
    Devices,
    RestoreCommand,
    LogChainValid = CASE WHEN BackupType='LOG' THEN @LogsChainValid ELSE NULL END
FROM #RestoreChain
ORDER BY StepNumber;
...
IF @Execute = 1
BEGIN
    PRINT 'Beginning execution of restore chain...';
    DECLARE @Step int = 1;
    WHILE @Step <= (SELECT MAX(StepNumber) FROM #RestoreChain)
    BEGIN
        SELECT @Cmd = RestoreCommand FROM #RestoreChain WHERE StepNumber = @Step;
        PRINT 'Executing Step ' + CAST(@Step AS varchar(10));
        BEGIN TRY
            IF @Debug = 1
                PRINT @Cmd;
            ELSE
        ...
        END TRY
        BEGIN CATCH
            PRINT 'ERROR at Step ' + CAST(@Step AS varchar(10)) + ': ' + ERROR_MESSAGE();
            RAISERROR('Aborting due to restore failure.',16,1);
            RETURN;
        END CATCH
        SET @Step += 1;
    END
    PRINT 'Restore chain completed.';
END
ELSE
    PRINT 'Execution skipped (script-only mode). Set @Execute=1 to run.';