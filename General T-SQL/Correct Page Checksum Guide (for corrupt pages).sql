...
DECLARE @ShipCity_new_name nchar(12) = N'123456789abc'
...
BEGIN try
	SELECT -- @PageNo = 
	CONVERT(INT,PARSENAME(REPLACE(sys.fn_PhysLocFormatter(%%PhysLoc%%) ,':','.'),'2')) FROM Orders WHERE orderid=10255	
END TRY
BEGIN CATCH
...
DECLARE @DataLength INT = DATALENGTH(@ShipCity_new_name)

SELECT @DataLength [Data Length] --, (select DATALENGTH(N'tabrizz'))

DECLARE @x varBinary(4000)
SET @x = CONVERT(varBinary(4000) , @ShipCity_new_name)

-- 0x578 + 0x86 = 1534 decimal
DBCC WRITEPAGE('Northwind' , 1 , @PageNo , 1534 , @DataLength , @x , 1) WITH NO_INFOMSGS
...
SELECT @correct_checksum_reordered [Correct Checksum Reordered]
