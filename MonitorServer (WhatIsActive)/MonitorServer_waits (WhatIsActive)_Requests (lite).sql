...
-- Description:			<monitorserver_waits (WhatIsActive)_Requests (lite)>
-- =============================================

-- For information please refer to the README.md

DROP FUNCTION IF EXISTS fn_udtvf_elapsedtime
DROP FUNCTION IF EXISTS fn_udtvf_monitorserver_waits_lite
...
CREATE FUNCTION fn_udtvf_monitorserver_waits_lite
(
	@HADR_waits BIT = 1,
	@Parallelism_waits BIT = 1,
	@Mirroring_waits BIT = 1,
...
(
	...
	SELECT TOP 100 PERCENT
		...
		, IIF(r.blocking_session_id<>0, (SELECT '#'+count_blocked+' ::: '+hb.hb FROM hb2 hb WHERE hb.blocked=r.session_id), NULL) [#CountBlocked_HeadBlocker]
...
SELECT * FROM fn_udtvf_monitorserver_waits_lite(1,1,1,1,1)
