# 🗂️ Restore Script Builder (msdb)

## How the pieces work together
1. `dbo.usp_build_restore_script` scans every online user database and, for each one, calls `dbo.usp_build_one_db_restore_script` to assemble (and optionally execute) the full restore chain. It aggregates per-database status in a temp table for quick review.
2. `dbo.usp_build_one_db_restore_script` is the heavy lifter: it collects the latest full, differential, and log backups (optionally from remapped locations), validates the log chain, generates RESTORE commands with MOVE/STATS/STOPAT/RECOVERY options, wraps them in TRY/CATCH timing + summary reporting, and can emit both plain T-SQL and SQLCMD-friendly scripts.
3. Helper functions support the inner procedure: `fn_SplitStringByLine` splits scripts for cursor-based assembly, `udf_BASE_NAME` and `udf_PARENT_DIR` extract path parts, and `user_dm_os_file_exists` checks file presence via `sys.dm_os_enumerate_filesystem`.

---

## 1) Bulk dispatcher: `dbo.usp_build_restore_script` (136 lines)
- Iterates through online user databases (excluding system DBs and `ReportServerTempDB`).
- Calls `dbo.usp_build_one_db_restore_script` for each database, passing through restore options (paths, STOPAT, REPLACE, RECOVERY, verbosity, execution flag, etc.).
- Tracks per-database success/failure with timestamps in `#Results` and outputs the summary grid.
- Accepts pre/post scripts and SQLCMD connection string to flow into the inner builder.

<details>
<summary>(click to expand) The complete 136-line script:</summary>
...
## 2) Per-database restore builder: `dbo.usp_build_one_db_restore_script` (955 lines)
- Assembles restore chains for a single database with optional STOPAT and MOVE overrides.
- Scans msdb backup history for latest full, optional differential, and sequential log backups; supports remapping backup locations and optional on-disk existence checks.
- Validates log chain continuity and warns if gaps exist; builds ordered restore steps with STATS/RECOVERY/REPLACE choices.
- Generates xp_create_subdir statements (optional), wraps restore steps in TRY/CATCH with elapsed-time tracking and summary output, and emits both plain T-SQL and SQLCMD-friendly scripts.
- Accepts pre/post restore scripts and can execute vs. print based on `@Execute` and `@Verbose`.

<details>
<summary>(click to expand) The complete 955-line script:</summary>
...
## 3) Helper functions (used by `dbo.usp_build_one_db_restore_script`)

### a) `dbo.fn_SplitStringByLine`
- Splits a multi-line NVARCHAR into rows with ordinal ordering.
- Used to emit scripts line-by-line into SQLCMD-friendly output.
...
### b) `dbo.udf_BASE_NAME`
- Returns the trailing component (file name) from a path or URL; trims trailing separators first.
...
### c) `dbo.udf_PARENT_DIR`
- Returns the parent directory portion of a path; yields empty string for URLs or inputs without separators.
...
### d) `dbo.user_dm_os_file_exists`
- Wraps `sys.dm_os_enumerate_filesystem` to check file existence, returning status plus an error description when the path pieces are missing.
...
RETURN
(
    SELECT NULL AS full_filesystem_path, NULL AS is_directory, NULL AS file_or_directory_name,
           1 AS Existence_Check_Failed,
           CONVERT(VARCHAR(500),'File/Directory "'+ @base_name + '" is not a valid OS or UNC path.') AS Existence_Check_Status_Desc
    WHERE @parent_dir = '' OR @base_name = ''
    UNION ALL
    SELECT fe.full_filesystem_path, fe.is_directory, fe.file_or_directory_name,
           0 AS Existence_Check_Failed, NULL AS Existence_Check_Status_Desc
    FROM sys.dm_os_enumerate_filesystem(@parent_dir, @base_name) fe
    WHERE @parent_dir <> '' AND @base_name <> ''
);
```
