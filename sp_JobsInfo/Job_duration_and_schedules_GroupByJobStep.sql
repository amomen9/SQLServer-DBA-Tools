...
SELECT
	...
	iif(dt.step_id=0,'job', convert(varchar(10),dt.step_id)) [job/step_id],
	--dt.step_id,
	[Job&Sched_enabled],	
...
(
	...
	JOIN login_roles lr ON SUSER_SNAME(j.owner_sid) = lr.[Login Name]
			GROUP BY j.name,j.owner_sid, info.Job_Enabled,info.Schedule_Enabled, info.Frequency,info.Interday_Frequency, [lr].[Member of], jsh.step_id, j.start_step_id, j.job_id
) dt LEFT join dbo.sysjobsteps js2
ON js2.job_id = dt.job_id AND js2.step_id = dt.step_id
ORDER BY dt.Job_Name,dt.step_id 
