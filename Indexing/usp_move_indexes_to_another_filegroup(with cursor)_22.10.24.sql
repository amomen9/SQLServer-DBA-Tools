...
(
	...
	@source_schema_name_filter sysname = '',
	@source_object_name_filter sysname = '',
...
----- Variable Control ----------------------------------------------------------
	...
	IF @target_filegroup_or_partition_scheme_name IS NULL OR @target_filegroup_or_partition_scheme_name = ''
	BEGIN
	...
	SET @source_object_name_filter = IIF(@source_object_name_filter = '' OR @source_object_name_filter IS NULL, '%', @source_object_name_filter)
	IF  @source_schema_name_filter IS NOT NULL AND @source_schema_name_filter<>'' AND SCHEMA_ID(@source_schema_name_filter) IS NULL
	BEGIN
		RAISERROR('The specified schema name does not exist',16,1)
		RETURN 1
	END
	ELSE    
		SET @source_schema_name_filter = IIF(@source_schema_name_filter = '' OR @source_schema_name_filter IS NULL, '%', @source_schema_name_filter)
		...
		'
			...
			FROM		  
				(SELECT i.data_space_id, SCHEMA_NAME(t.schema_id) as SchemaName, OBJECT_NAME(i.object_id) TableName, i.object_id TableID
					...
					WHERE i.index_id = 0 and t.name like '''+@source_object_name_filter+''' and schema_name(t.schema_id) like '''+@source_schema_name_filter+'''
				) AS uind
		...
		'
			...
			FROM		  
				(
					...
					JOIN sys.indexes i 
					ON t.object_id = i.object_id AND t.is_ms_shipped=0
					WHERE i.index_id between @starting_index_id AND @ending_index_id AND t.name like '''+@source_object_name_filter+''' and schema_name(t.schema_id) like '''+@source_schema_name_filter+'''
...
EXEC dbo.usp_move_indexes_to_another_filegroup @DatabaseName = 'dbWarden',                              -- sysname
                                               @starting_index_id = 0,                            -- tinyint
                                               @ending_index_id = NULL,                           -- tinyint
                                               @source_object_name_filter = '%Archive',
											   @source_schema_name_filter = '',
											   @source_filegroup_or_partition_scheme_name = '',   -- sysname	Empty or NULL yields all filegroups except the target filegroup.
                                               @target_filegroup_or_partition_scheme_name = 'FG_DbWardenArchive', -- sysname
                                               @SORT_IN_TEMPDB = 0,                               -- bit
                                               @STATISTICS_NORECOMPUTE = 1,                       -- bit
                                               @STATISTICS_INCREMENTAL = 1,                       -- bit
                                               @ONLINE = 1,                                       -- bit
                                               @MAXDOP = 1,                                    -- tinyint
                                               @DATA_COMPRESSION = 'DEFAULT',                     -- nvarchar(10)
                                               @DATA_COMPRESSION_PARTITIONS = NULL,               -- nvarchar(128)
                                               @FILESTREAM = 'OFF',                               -- nvarchar(20)
                                               @StartTime = @StartTime,                                 -- datetime
                                               @Retry_With_Less_Options = 1,                      -- bit
                                               @Create_or_Update_IndexTransferResults_Table = 0   -- bit
